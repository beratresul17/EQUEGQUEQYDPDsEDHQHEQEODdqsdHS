...
exports.run = async(client, message, args) => {
 ...
 let rol = message.mentions.roles.first() || message.guild.roles.get(args.join(' '))
  let newRole;
  let tworole;
  if (!rol) return message.channel.send(`** Otorol ayarlamanız için bir rol etiketlemeniz gerek.** ${prefix}otorol ayarla @Üye #kanal`)
  ...
  if (!otorolkanal) return message.channel.send(`** Otorol Ayarlamanız İçin Bir Rol Etiketlemeniz Gerek.** ${prefix}otorol ayarla @Üye #kanal`)
