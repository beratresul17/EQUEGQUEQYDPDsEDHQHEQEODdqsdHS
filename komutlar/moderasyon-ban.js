...
exports.run = async (bot, message, args) => {
  ...
  if (!message.member.hasPermission("BAN_MEMBERS"))
    return message.channel.send(
      "Strix | **Bu komudu kullanabilmek için `Üyeleri Yasakla` yetkisine sahip olmanız gerek.**"
    );
  let reason = args.slice(1).join(" ");

  if (!args[0])
    return message.channel.send(
      "Strix | Yasaklamak istediğiniz kullanıcıyı etiketleyiniz."
    );
  let user =
  ...
  while (!uwu) {
    ...
    if (choice !== "evet" && choice !== "e") {
      message.channel.send(
        "Strix | Lütfen sadece **`evet (e)`** veya **`hayır (h)`** ile cevap verin."
      );
    }
  ...
  if (uwu) {
    try {
      ...
      message.channel.send(
        `Strix | **${user.tag}** adlı kullanıcı sunucudan yasaklandı.`
      );

      user.send(
        `Strix | **${message.guild.name}** adlı sunucudan **banlandınız!**\n*Sebep:* \`\`\`${reason}\`\`\``
      );

      let motion = new moment.MessageEmbed()
        .setColor(0xffa300)
        .setAuthor(
          `Strix | ${user.username} adlı kişi yasaklandı!`,
          user.avatarURL || user.defaultAvatarURL
        )
        .addField(
          "Strix | Yasaklanan Kullanıcı",
          `${user.tag}-[${user.id}]`,
          true
        )
        .addField(
          "Strix | Yasaklayan Yetkili",
