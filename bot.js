...
.setDescription(`<a:hypesquad1:750076071721828452> <@!${message.author.id}> adlı kullanıcı tarafından <#${message.channel.id}> kanalına gönderilen mesaj silindi!\n\nSilinen Mesaj: **${message.content}**`)
  .setFooter("Strix Bot | Log Sistemi")
...
if (!modlogkanal) return;
    if (channel.type === "text") {
                ...
                .setFooter(`Strix Bot | Log Sistemi Kanal ID: ${channel.id}`)
              ...
              if (!message.member.hasPermission("ADMINISTRATOR")) {
                  ...
                  if (uyarisayisi === null) {
                      let uyari = new Discord.RichEmbed()
                          .setColor("#ffd100")
                          .setFooter('Strix BOT', client.user.avatarURL)
                  ...
                  if (uyarisayisi === 3) {
                      message.delete();
                      await kullanici.ban({
                          reason: `Strix Reklam Kick Sistemi`,
                      })
...
if (!role) {
  member.addRole(member.guild.roles.get(otorol))
                        ...
                        .setColor('#ffd100') 
                        .setFooter(`Strix Otorol Sistemi`)
